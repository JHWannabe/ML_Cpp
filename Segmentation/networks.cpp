...
UNetImpl::UNetImpl(mINI::INIStructure& ini) {
    ...
    size_t num_downs = (size_t)(std::log2(std::stold(ini["General"]["size_w"])));
    bool use_dropout = !stringToBool(ini["Network"]["no_dropout"]);

    UNetBlockImpl blocks, fake;
    blocks = UNetBlockImpl({ feature * 8, feature * 8 }, std::stol(ini["General"]["latent_space"]), /*submodule_=*/fake, /*outermost_=*/false, /*innermost=*/true);
    ...
    blocks = UNetBlockImpl({ std::stol(ini["General"]["input_channel"]), std::stol(ini["General"]["class_num"])}, feature, /*submodule_=*/blocks, /*outermost_=*/true);
