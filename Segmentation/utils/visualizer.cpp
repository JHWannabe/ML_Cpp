...
cv::Mat visualizer::draw_detections(const torch::Tensor image, const std::tuple<torch::Tensor, torch::Tensor> label, const std::vector<std::string> class_names, const std::vector<std::tuple<unsigned char, unsigned char, unsigned char>> label_palette, const std::pair<float, float> range) {
    ...
    if (BB_n > 0) {
        for (size_t b = 0; b < BB_n; b++) {
            ...
            // (4.1) Get parameters
            id = ids[b].item<int64_t>();		// id[0,CN]
            cx = coords[b][0].item().toFloat(); // cx[0.0,1.0]
            cy = coords[b][1].item().toFloat(); // cy[0.0,1.0]
            w = coords[b][2].item().toFloat();   // w[0.0,1.0]
            h = coords[b][3].item().toFloat();   // h[0.0,1.0]
...
void visualizer::graph::plot(const float base, const std::vector<float> value) {
    ...
    if (this->flag) {
        FILE* gp;
        gp = _popen("gnuplot -persist -e", "w");
        if (gp == nullptr) {
            // gnuplot 실행 오류 처리
            std::cerr << "Error: gnuplot could not be started." << std::endl;
            return;
        }
    ...
    //if (this->flag)
    //{
    //    std::ofstream gnuplotFile("gnuplot_command.gp");
    //    gnuplotFile << "set terminal png\n";
    //    gnuplotFile << "set ouput" << this->graph_fname << "'\n";
    //    gnuplotFile << "plot ";
    //    for (size_t i = 0; i < this->label.size() - 1; i++) {
    //        gnuplotFile << "'" << this->data_fname << "' using 1:" << (i + 2) << " ti '" << this->label.at(i) << "' with lines, ";
    //    }
    //    gnuplotFile << "'" << this->data_fname << "' using 1:" << (this->label.size() + 1) << " ti '"
    //        << this->label.at(this->label.size() - 1) << "' with lines\n";
    //    gnuplotFile.close();
    //    // gnuplot 실행 (Windows 환경용)
    //    system("gnuplot gnuplot_commands.gp");
    //    // 필요시 gnuplot 명령어 파일 삭제
    //    remove("gnuplot_commands.gp");
    //}
