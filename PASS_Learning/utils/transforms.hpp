...
#define transforms_Normalize std::make_shared<transforms::NormalizeImpl>
    ...
    // ---------------------------------------------------------------
    // namespace{transforms} -> class{CenterCropImpl}(ComposeImpl)
    // ---------------------------------------------------------------
#define transforms_CenterCrop std::make_shared<transforms::CenterCropImpl>
    class CenterCropImpl : public ComposeImpl {
    private:
        cv::Size size;
        int interpolation;
    public:
        CenterCropImpl() {}
        CenterCropImpl(const cv::Size size_);
        bool type() override { return CV_MAT; }
        void forward(cv::Mat& data_in, cv::Mat& data_out) override;
    };
