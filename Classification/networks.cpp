...
MC_ResNetImpl::MC_ResNetImpl(mINI::INIStructure& ini) {
    ...
    size_t n_layers = std::stol(ini["Network"]["n_layers"]);
    if (n_layers == 18) {
        basic_block = true;
        cfg = { 2, 2, 2, 2 };
    }
    ...
    this->inplanes = std::stol(ini["Network"]["nf"]);

    // First Downsampling
    this->first = nn::Sequential(
        nn::Conv2d(nn::Conv2dOptions(/*in_channels=*/std::stoll(ini["General"]["input_channel"]), /*out_channels=*/this->inplanes, /*kernel_size=*/7).stride(2).padding(3).bias(false)),  // {C,224,224} ===> {F,112,112}
    ...
    register_module("conv1", this->first[0]);
    register_module("bn1", this->first[1]);
    ...
    // Classification
    //this->classifier = nn::Sequential(nn::Linear(/*in_channels=*/feature * 8 * expansion, /*out_channels=*/std::stol(ini["General"]["class_num"])));  // {8F*E} ===> {CN}
    this->classifier = nn::Linear(/*in_channels=*/feature * 8 * expansion, /*out_channels=*/std::stol(ini["General"]["class_num"]));  // {8F*E} ===> {CN}
    register_module("fc", this->classifier);
...
BasicBlockImpl::BasicBlockImpl(const size_t inplanes, const size_t planes, const size_t stride) {
    ...
    register_module("conv1", this->layerA[0]);
    register_module("bn1", this->layerA[1]);
    ...
    register_module("conv2", this->layerB[0]);
    register_module("bn2", this->layerB[1]);
...
BottleneckImpl::BottleneckImpl(const size_t inplanes, const size_t planes, const size_t stride) {
    ...
    register_module("conv3", this->layerC[0]);
    register_module("bn3", this->layerC[1]);
